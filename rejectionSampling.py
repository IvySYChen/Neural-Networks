...
def rejection_sampled_expectation(target, proposal, f=identity, num_samples=2000):
    ...
    M = proposal.max_pdf()/target.max_pdf()-0.01
    #print(proposal.max_pdf(),target.max_pdf())
    
    samples = []
    n=0
    for i in range(num_samples):
      v = proposal.sample() # sample a single value from the distribution
      x = proposal.pdf(v)
      y = target.pdf(v)
    ...
    #print("reject=",n, "mean=",mean(samples))
    return mean(samples)
...
def main():
    ...
    #rejection_sampled_expectation(trunc, U33)
    #rejection_sampled_expectation(trunc, untrunc_2)
